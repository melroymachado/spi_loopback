...
int32_t DRV_SPI0_ISRMasterRM8BitTasks ( struct DRV_SPI_OBJ * dObj )
{
    ...
    do {
        ...
        {   
            ...
            if (((dObj->rxEnabled) && SYS_INT_SourceStatusGet(INT_SOURCE_SPI_1_RECEIVE)) ||
                ((dObj->txEnabled) && SYS_INT_SourceStatusGet(INT_SOURCE_SPI_1_TRANSMIT)) ||
...
int32_t DRV_SPI1_ISRSlaveRM8BitTasks ( struct DRV_SPI_OBJ * dObj )
{
    ...
    do {
        ...
        if (dObj->currentJob != NULL)
        {
            ...
            /* Interrupts should immediately become active again if they're in a fired condition */
            if (((dObj->rxEnabled) && SYS_INT_SourceStatusGet(INT_SOURCE_SPI_2_RECEIVE)) || ((dObj->txEnabled) && SYS_INT_SourceStatusGet(INT_SOURCE_SPI_2_TRANSMIT)) || (SYS_INT_SourceStatusGet(INT_SOURCE_SPI_2_ERROR)))
            {
                /* Interrupt would fire again anyway so we should just go back to the start*/
                continueLoop = true;
                continue;
